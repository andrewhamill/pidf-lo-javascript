...
export const instanceOfElement = (obj: any): boolean => {
  if (isBrowser) return obj instanceof Element;
  ...
  else return !!(obj.nodeName && obj.localName);
};
export const getElementsByLocalName = (
  element: Element,
  localName: string,
  result: Element[] = []
) => {
...
const addXMLProlog = (xmlDocument: string) =>
